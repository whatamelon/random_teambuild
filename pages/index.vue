<template> 
  <div class="md:h-screen h-full w-full md:flex block">
    <div class="w-full md:w-2/5 h-full md:h-full px-4 py-4 md:px-20 md:py-12 bg-white">
      ...
      <h2 class="prose prose-2xl font-bold text-black mt-10">{{ $t('teamLeaders') }}</h2>
      <div class="w-full flex space-x-4 pt-2">
      ...
      <div class="w-full flex space-x-4 pt-48 md:pt-24 pb-10 md:pb-0">
    ...
    <div v-if="chunkedList.length > 0" class="w-full md:w-3/5 h-full bg-neutral-700 px-4 pt-16 pb-4 md:px-20 md:pt-12 md:pb-12">
    ...
    <div v-else class="w-full md:w-3/5 h-full bg-neutral-700 flex">
      <p @click="goToExample()" class="py-20 md:py-0 text-[2rem] md:text-[3rem] font-bold text-center my-auto mx-auto h-fit hover:text-orange-500 cursor-pointer"> <i class="ri-question-line"></i> {{ $t('howToUse') }}</p>
    </div>
  </div>
</template>
